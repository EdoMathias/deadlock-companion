...
class ReleaseNotesService {
  ...
  async getReleaseNoteForVersion(
    version: string,
  ...
  hasViewedReleaseNote(entry: ReleaseNoteEntry | null): boolean {
    ...
    return (
      payload.version === entry.version && payload.timestamp === entry.timestamp
    );
  }
  ...
  private async fetchReleaseNotes(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Release notes request failed with status ${response.status}`,
        );
      }
  ...
  private getViewedPayload(): ViewedReleaseNotePayload | null {
    try {
      ...
      this.logger.warn(
        'Failed to parse stored release notes viewed payload',
        error,
      );
      return null;
    }
